...
| File | Việc | Sửa khi |
|---|---|---|
| **`huong-dan-giai-doan.ts`** | **Văn bản hướng dẫn nghiệp vụ** của 6 bước, chép nguyên văn quy trình "TM-QT Mua hàng (HP CONS)" trên Base.vn. Kèm ba hàm đổi qua lại giữa văn bản thuần và cấu trúc (`huongDanThanhVanBan` · `vanBanThanhHuongDan` · `huongDanHienThi`) | Công ty ban hành quy trình mới. 🔴 **Người lập trình không tự viết lại cho gọn** — người dùng đối chiếu với quy trình giấy.<br>📌 Từ 15/09/2026 **cấp quản lý sửa được nội dung ngay trên giao diện** (nút *Sửa nội dung hướng dẫn* trong hộp ⓘ, quyền `suaPODaChot`). Bản sửa lưu ở `cauHinh.huongDanTuyChinh`, **bản gốc trong file này vẫn là mặc định và khôi phục được** bằng cách xoá khoá — nên đừng chép bản sửa ngược vào đây |
...
| **`xuat-cong-no-excel.ts`** | **Xuất danh sách công nợ ra .xlsx theo khung thời gian** (25/09/2026) — 2 trang tính (theo đơn · theo từng tờ hoá đơn). Lọc theo ngày hoá đơn, đơn chưa có hoá đơn theo ngày lập PO (`locCongNoTheoKhung`, hàm thuần). Con số lấy nguyên từ `congNoTheoDonHang`, không tính lại | Tệp thiếu đơn / sai khung ngày |
...
| **`dat-ma-don-hang.ts`** | Sinh số đơn **`DMH[năm 2 số][STT 4 số]`**, vd `DMH260001` (Ban lãnh đạo 23/08/2026). ⚠️ **Mô tả cũ ở đây ghi `[mã dự án]-PO-[STT]` — đã lỗi thời, sửa 25/08.** Dãy số chạy theo **NĂM**, không theo dự án, nên nhìn số đơn **không suy ra được công trình** nữa. Năm lấy theo **ngày lập đơn**, không theo hôm nay. 🔴 Lấy **số lớn nhất đã dùng rồi +1**, không đếm số đơn hiện có (đếm rồi +1 là sinh mã TRÙNG khi có đơn bị bỏ). 📌 Mã `DMH` **chưa có trong danh mục Thông báo 09/2026** — việc xin bổ sung còn nợ | Mã đơn trùng, sai dạng, hoặc đổi ký hiệu `DMH` |
| **`dat-ma-nha-cung-cap.ts`** | Sinh mã nhà cung cấp **`NC[STT 4 số]`**, vd `NC0001` (Ban lãnh đạo 25/08/2026: *"Mã NCC sẽ tự động sinh ra… cấu trúc NC+0000. Và mục này sẽ không được sửa"*). 🔴 Lấy **số lớn nhất đã dùng rồi +1** — danh mục **có** chức năng xóa nên "xóa giữa rồi thêm lại" là ca có thật, đếm rồi +1 sẽ cấp trùng. 📌 `^NC(\d+)$` **không ăn nhầm** dãy cũ `NCC0001` (chữ C thứ ba không khớp) nên hai dãy đếm độc lập; vòng chống trùng vẫn nhìn cả hai. ⚠️ Đây là **mã danh mục nội bộ**, không phải mã hồ sơ theo Thông báo 09/2026 | Mã NCC trùng, sai dạng, hoặc muốn đổi tiền tố `NC` |
...
| **`phieu-xuat-kho.ts`** | Luật riêng của **Mẫu PO-03 — Phiếu xuất kho** (Sếp 26/09/2026): bố cục biểu mẫu (ghi ở đầu tệp) · hằng `MA_LOAI_PHIEU_XUAT_KHO` **[CHỜ CHỐT]** (để `null` → "Số" phiếu dùng đúng số đơn; không tự đặt mã loại, quy tắc E-6) · `soPhieuXuatKho` · `dongNgayThangNam` · `vuongMacXuatExcelTheoMau` (lý do khoá nút Excel ở PO-03) | Số phiếu xuất kho sai, nút Excel PO-03 khoá/mở sai |
...
| **`dong-do-theo-app-request.ts`** | Sếp 26/09/2026: App Request **xoá đề xuất** → cả họ phiếu mang mã đó sang **Thất bại** (trừ phiếu đã Hoàn thành). Đơn hàng giữ nguyên, lập danh sách + tin báo đỏ cho Trưởng bộ phận. Route gọi: `app/api/app-request/de-nghi-da-xoa/route.ts` | Xoá bên App Request mà hồ sơ không sang Thất bại; đụng nhầm phiếu khác mã; gọi lại bị nhân đôi lịch sử |
...
| **`gom-cong-trinh.ts`** | Khoá nhóm theo tên công trình + tên công trình của PO — dùng chung màn Công nợ và Theo dõi (25/09/2026) | Công trình bị gộp nhầm / tách đôi |
| **`tach-khi-giao-viec.ts`** | 🔴 **Tách phiếu NGAY LÚC GIAO VIỆC** (26/09/2026): mỗi người một phiếu con mã cố định `<gốc>__<uid>`, người giao sau cùng giữ phiếu gốc (gộp phiếu con cũ về), giao cả phiếu cho 1 người không tách, chặn chuyển/bỏ dòng đã có báo giá/PO. Thay đoạn tách ngầm 22/08 đã sinh bản lặp | Giao việc sinh phiếu thừa, dòng mất, người nhận có 2 phiếu |
...
| **`nhan-ban-de-nghi.ts`** | Nhân bản / tách phiếu: mã và tên bản sao (`maBanSaoTiepTheo` · `tenBanSaoTheoMa`), quan hệ cha–con **một cấp** (`phieuGocCua` · `cacBanTachCua`), dấu mờ dòng đã nhân bản (`dongDaNhanBanSang`), phương án tách theo phân công, và **`dungBanNhanBan`** — nơi DUY NHẤT dựng bản sao. 🔴 **Làm sạch bản sao ở đây, không ở `kho-du-lieu.tsx`** (Sếp 15/09/2026: *"a cần làm sạch tất cả khi trả về bước 2"*): bản copy bắt đầu lại ở bước ② nên bỏ `tepGiaiDoan` · `binhLuan` · `lyDoThieuChungTu` · `lyDoThatBai` · `luuTru` và việc đã tích từ bước ③ trở đi; **giữ** `deNghiGocId` · `sttDongGoc` · `loaiHoSo` · `taiLieu` (hồ sơ đầu vào). ⚠️ Chỉ bỏ THAM CHIẾU tệp — nội dung ở `3-du-lieu/kho-tep.ts` dùng chung `id` với phiếu gốc, xoá là phiếu gốc mất chứng từ | Bản nhân bản mang theo chứng từ của phiếu khác, hoặc mất liên kết về phiếu gốc |
| **`xoa-de-nghi.ts`** | Luật xoá hẳn đề nghị (`lyDoKhongXoaDeNghi` · `phanLoaiXoaDeNghi`) — xét CẢ TẬP đã chọn: có báo giá/đơn hàng thì chặn, phiếu cha chỉ xoá được khi chọn cả bản con. Sếp 26/09/2026 bỏ nút xoá toàn bộ, thay bằng chọn từng phiếu ở Cài đặt quy trình | Xoá làm báo giá / đơn hàng / bản con mồ côi |
| **`giu-ban-ghi-moi.ts`** | **Giữ bản ghi máy này vừa tạo cho tới khi thấy nó trên máy chủ** — sinh ra từ sự cố MẤT DỮ LIỆU thật 15/09/2026 (Sếp báo 19:33: lập đơn xong → *"Không tìm thấy đơn đặt hàng"*; đo trên `hpcons-portal` thấy thiếu cả DMH260001/260003/260004 mà app **không hề có chức năng xoá đơn**). Gồm `idVuaTaoTaiMay` · `idCanGhepLai` · `ghepBanChuaLenMayChu` · `soSauAnhChup` · `quaHanDongBo`, và nhịp thử lại khi ghi kho chung hỏng (`khoangChoGhiLai` · `conDuocGhiLai`). 🔴 **Phân biệt "bản mới chưa lên server" với "bản người khác cố ý xoá" bằng một CỬA MỘT CHIỀU**: chỉ giữ id máy này tạo **trong phiên này** và **chưa từng có trong bất kỳ ảnh chụp nào từ máy chủ**; thấy một lần là thôi theo dõi vĩnh viễn. Bỏ vế nào cũng biến bản vá thành lỗi nặng hơn — xoá đơn ở máy A thì máy B dựng nó sống lại mãi | Bản ghi vừa lập biến mất khỏi màn hình; hoặc ngược lại: xoá rồi mà cứ sống lại |
| **`ten-the-de-nghi.ts`** | Ghép **dòng tiêu đề của thẻ** trên bảng quy trình: mã đầu thẻ + số hợp đồng + tên công trình + phần tên tự đặt (đã trừ chữ lặp). 🔴 Đuôi `(copy N)` **chỉ cắt khi mã đầu thẻ đã mang sẵn nó** — hồ sơ từ App Request in `maDeXuatAppRequest` trùng hệt phiếu gốc, cắt đuôi là hai thẻ giống hệt nhau (Sếp 15/09/2026, đo được trên bảng chạy thật) | Hai thẻ trông giống nhau; thẻ lặp chữ; muốn đổi khuôn dòng tiêu đề |
