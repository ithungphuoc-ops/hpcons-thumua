...
| **Mã đơn hàng sai dạng / trùng nhau** (`DMH260001`) | `2-quy-trinh/dat-ma-don-hang.ts` → `maDonHangTiepTheo`, gọi từ `3-du-lieu/kho-du-lieu.tsx` → `themDonHang`. ⚠️ **Dòng này trước ghi mẫu cũ `260001-HPCS-PO-001` — đã lỗi thời, sửa 25/08.** Từ 23/08/2026 số đơn là `DMH[năm 2 số][STT 4 số]`, chạy theo **NĂM** chứ không theo dự án. 🔴 Lấy **số lớn nhất đã dùng rồi +1**, không đếm số đơn hiện có |
| **Mã nhà cung cấp trùng / sai dạng / muốn cho sửa tay** (`NC0001`) | `2-quy-trinh/dat-ma-nha-cung-cap.ts` → `maNhaCungCapTiepTheo`, gọi từ `3-du-lieu/kho-du-lieu.tsx` → `themNhaCungCap`. 🔴 **Mã do TẦNG GHI cấp, không cấp ở giao diện** (Ban lãnh đạo 25/08/2026) — cấp ở giao diện thì hai người mở hộp cùng lúc thấy **cùng một mã**. Ô mã trong hộp thêm NCC là **chỉ đọc**, hiện khuôn `NC0000 (app tự cấp)`. `themNhaCungCap` trả union `{ loi } \| { ma }` để nơi gọi biết mã vừa cấp mà điền vào đơn |
...
| **Mẫu PO-03 — Phiếu xuất kho** (26/09/2026) sai bố cục / thiếu ô | Tờ in: `1-giao-dien/thanh-phan-nghiep-vu/to-phieu-xuat-kho-a4.tsx` (được `to-don-mua-hang-a4.tsx` rẽ nhánh gọi) · Xem nhanh trên form: `khoi-tong-quan-phieu-xuat-kho.tsx` · Ô nhập: `form-lap-don-mua-hang.tsx` (khối "Thông tin phiếu xuất kho") · Luật + mã (**`XK260001`**, Sếp chốt 26/09/2026): `2-quy-trinh/phieu-xuat-kho.ts` + số cấp ở `2-quy-trinh/dat-ma-don-hang.ts` → `thamSoCapSoDon` · Trường dữ liệu: `DonDatHang.*XuatKho` / `khoXuat` / `diaDiemKhoXuat` trong `3-du-lieu/kieu-du-lieu.ts`. Chế độ SỬA: `kho-du-lieu.tsx` → `mocSuaPhieuXuatKho` (26/09/2026). PO-03 không nhập giá, không gửi phần giá khi sửa |
...
| **Giao việc tách phiếu sai** (sinh phiếu thừa, người có 2 phiếu, dòng không mờ, chuyển việc bị chặn) | Luật: `2-quy-trinh/tach-khi-giao-viec.ts` → `apDungGiaoViec` / `apDungRutDong` · Tầng ghi: `3-du-lieu/kho-du-lieu.tsx` → `phanBoDong` / `chuyenViecDong` / `boPhanBoDong` · Dòng mờ chỉ Trưởng bộ phận thấy: `bang-phan-bo.tsx` → `tienDo` |
...
| **Đề xuất bị xoá bên App Request** mà hồ sơ không sang Thất bại / sang nhầm | Cửa nhận: `app/api/app-request/de-nghi-da-xoa/route.ts` (phiên nghiệp vụ, KHÔNG thuộc vùng cấm) · Luật: `2-quy-trinh/dong-do-theo-app-request.ts` → `apDungXoaTuAppRequest` |
...
| 🔴 **"Không lập được bảng báo giá đầu tiên"** / mất đường vào module Báo giá | **Cập nhật 12/09/2026 — dòng này trước đây ghi SAI.** Bản cũ nói lối vào là mục *Lập bảng báo giá* trong menu ⋯ trên thẻ. Mục đó **đã bỏ 12/09/2026** theo chỉ đạo Ban lãnh đạo, và nó vốn **đang hỏng** (thẻ đứng ở cột ② thì đích trùng cột hiện tại → `xuLyTha` thoát im lặng, bấm không có gì xảy ra). **Hai lối vào đang sống:** ① mục *Chuyển sang giai đoạn kế tiếp* trong chính menu ⋯ (gọi `onTha` → `xuLyTha` → `quyetDinhKeoTha`, giữ nguyên mọi chốt); ② nút *Trình xét duyệt báo giá* ở bước ② trang chi tiết — từ 20/08/2026 nút này **tự lập bảng** nếu hồ sơ chưa có bảng nào (`trinhXetDuyetBaoGiaChoDeNghi`). Cả hai bấm được trên điện thoại. ⚠️ Kéo thẻ **đã tắt hoàn toàn từ 27/08/2026** (`keoThaDuoc={false}`) nên đừng tính nó là lối vào |
...
| Sai **nhóm theo công trình** (màn Công nợ, màn Theo dõi) — hai công trình bị gộp nhầm / một công trình bị tách đôi | `2-quy-trinh/gom-cong-trinh.ts` → `khoaCongTrinh`, `gomTheoCongTrinh` (🔴 một luật cho cả hai màn) |
...
| **Bản nhân bản mang theo tệp / bình luận / lý do của phiếu gốc** | `2-quy-trinh/nhan-ban-de-nghi.ts` → **`dungBanNhanBan`** — nơi duy nhất dựng bản sao (Sếp 15/09/2026: *"a cần làm sạch tất cả khi trả về bước 2"*). `3-du-lieu/kho-du-lieu.tsx` → `nhanBanDeNghi` chỉ còn tra phiếu · kiểm quyền · cấp id/mã. 🔴 Chỉ bỏ THAM CHIẾU tệp, **không xoá nội dung** ở `3-du-lieu/kho-tep.ts` (hai bản dùng chung `id`, xoá là phiếu gốc mất chứng từ). Bài kiểm: `npm run kiem-luat` |
| **Xoá đề nghị** (Cài đặt quy trình → Vùng nguy hiểm, chỉ quản trị) | `2-quy-trinh/xoa-de-nghi.ts` (luật) · `3-du-lieu/kho-du-lieu.tsx` → `xoaNhieuDeNghi` · giao diện `1-giao-dien/thanh-phan-nghiep-vu/khoi-xoa-du-lieu-chay-thu.tsx`. 🔴 Nút xoá toàn bộ đã bỏ 26/09/2026 — đừng dựng lại |
| **Hai thẻ kanban trông giống hệt nhau** sau khi nhân bản / thẻ lặp chữ ở dòng tiêu đề | `2-quy-trinh/ten-the-de-nghi.ts` → `manhTenTheDeNghi` (thẻ gọi vào đây, **không tự ghép trong JSX**). 🔴 Đuôi `(copy N)` chỉ cắt khi **mã đầu thẻ đã mang sẵn nó** — hồ sơ App Request in `maDeXuatAppRequest` trùng hệt phiếu gốc nên cắt đuôi là mất dấu hiệu phân biệt cuối cùng (Sếp 15/09/2026) |
...
| ★ **Tick quyền riêng** từng người / nhiều người (Sếp 26/09/2026): tick không ăn, người được tick vẫn không thấy/làm được, trưởng bộ phận bị báo "không trao được" | Áp quyền: `4-phan-quyen/quyen-rieng.ts` → `apDungQuyenRieng` (gọi ở dòng cuối `tinhQuyen`) · Ai trao được gì cho ai: `4-phan-quyen/luat-phan-quyen.ts` → `vuongMacTraoQuyen` · Cửa đọc/ghi: `app/api/quyen-rieng/route.ts` (collection `tm_quyen_rieng`, chỉ Admin SDK) · Đọc lúc tải trang: `4-phan-quyen/nguoi-dung-hien-tai.tsx`. 🔴 Chưa tick riêng = theo chức danh; người được đổi thấy thay đổi ở lần **tải trang kế tiếp** · Ô tick "Lập đơn mua hàng" còn gác 3 quyền theo hồ sơ (`quyen-theo-ho-so.ts` → `tickChoLamThuMua`) · Người bị bỏ "Vào app" không hiện trong danh sách Giao việc (`4-phan-quyen/dung-nguoi-khong-vao-app.ts`) |
